...
export class PalestrasVideosProfissional {
    ...
    renderHTML() {
        return `<div class="dashboard-container media-library-page"><div id="menuContainer"></div><main class="main-content media-library-main">
            ...
            <section id="painelVideos" class="media-tab-panel" hidden><div class="media-toolbar"><label><span>Buscar conteúdo</span><input id="buscaConteudo" type="search" placeholder="Título, descrição ou categoria"></label><label><span>Visibilidade</span><select id="filtroVisibilidade"><option value="">Todas</option><option value="global">Global</option><option value="privado">Privado</option><option value="exclusivo">Exclusivo</option></select></label></div><section id="listaConteudos" class="media-content-grid"><div class="media-empty"><div class="media-loader"></div><p>Carregando biblioteca...</p></div></section></section>
        </main><div class="media-create-actions"><button id="btnNovaPalestra" class="btn-primary btn-expand" title="Criar palestra"><span>+</span><span class="btn-text">Criar palestra</span></button><button id="btnNovoConteudo" class="btn-primary btn-expand" title="Novo vídeo ou material" hidden><span>+</span><span class="btn-text">Novo vídeo ou material</span></button></div><div id="modalConteudo" class="media-modal" hidden>${this.renderFormulario()}</div><div id="modalPalestra" class="media-modal" hidden>${this.renderFormularioPalestra()}</div><div id="modalSalaJitsi" class="media-modal lecture-room-modal" hidden><div class="lecture-room-dialog"><header><div><span class="media-eyebrow">Sala de palestra</span><h2 id="tituloSalaJitsi">Palestra ao vivo</h2></div><button id="btnFecharSala" aria-label="Sair da sala">×</button></header><div id="jitsiContainer"></div></div></div></div>`;
    }
    ...
    abrirModalPalestra(palestra = null) {
        ...
        document.getElementById('termoResponsabilidadePalestra').checked = false;
    ...
    renderCard(c) {
        ...
        return `<article class="media-card">${preview}<div class="media-card-body"><div class="media-card-meta"><span class="media-badge is-${this.escape(c.visibilidade)}">${label}</span><span>${this.escape(c.categoria || 'Sem categoria')}</span></div><h3>${this.escape(c.titulo)}</h3><div class="media-card-footer"><span class="media-footer-description" title="${this.escape(c.descricao || 'Sem descrição.')}">${this.escape(c.descricao || 'Sem descrição.')}</span><small>${this.formatarData(c.criado_em)}</small><div><a href="${this.escape(c.arquivo_url)}" target="_blank" rel="noopener" title="Abrir">↗</a><button data-editar-conteudo="${c.id}" title="Editar">✎</button><button data-excluir-conteudo="${c.id}" title="Excluir" class="is-danger">⌫</button></div></div></div></article>`;
    }
    ...
    attachEvents() {
        document.querySelectorAll('[data-media-tab]').forEach(button => button.onclick = () => {
            ...
            document.getElementById('btnNovoConteudo').hidden = button.dataset.mediaTab !== 'videos';
        });
