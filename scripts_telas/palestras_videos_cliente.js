...
import { db, collection, getDocs, doc, getDoc, obterAcessoJaaS } from '../0_firebase_api_config.js';
...
export class PalestrasVideosCliente {
    ...
    async carregar() {
        try {
            ...
            this.palestras = palestrasSnapshot.docs.map(d => ({ id:d.id, ...d.data() })).filter(p => p.visibilidade === 'global' || (p.visibilidade === 'privado' && this.profissionais.includes(p.profissional_login)) || (p.visibilidade === 'exclusivo' && this.profissionais.includes(p.profissional_login) && (p.pacientes_exclusivos || []).includes(this.userInfo.login))).sort((a,b) => new Date(a.inicio_em) - new Date(b.inicio_em));
            this.atualizarLista();
    ...
    renderPalestra(p) { const s=this.statusPalestra(p),i=new Date(p.inicio_em),data=i.toLocaleDateString('pt-BR',{day:'2-digit',month:'short',year:'numeric'}),hora=i.toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'});return `<article class="lecture-card"><div class="lecture-date"><strong>${i.getDate().toString().padStart(2,'0')}</strong><span>${i.toLocaleDateString('pt-BR',{month:'short'}).replace('.','')}</span></div><div class="lecture-info"><div class="lecture-meta"><span class="lecture-status is-${s.chave}">${s.label}</span><span class="media-badge is-${p.visibilidade}">${p.visibilidade==='global'?'Global':p.visibilidade==='exclusivo'?'Exclusiva':'Seus pacientes'}</span></div><h3>${this.escape(p.titulo)}</h3><p>${this.escape(p.descricao||'Sem descrição.')}</p><div class="lecture-details"><span>◷ ${data}, ${hora}</span><span>⌛ ${p.duracao_minutos||60} min</span><span>Por ${this.escape(p.profissional_nome)}</span></div></div><div class="lecture-actions">${s.chave!=='encerrada'?`<button class="media-primary-btn" data-entrar-palestra="${p.id}">${s.chave==='agendada'?'Entrar na sala':'Participar agora'}</button>`:''}</div></article>`; }
    async carregarJitsi(appId){if(window.JitsiMeetExternalAPI)return;await new Promise((resolve,reject)=>{const s=document.createElement('script');s.src=`https://8x8.vc/${encodeURIComponent(appId)}/external_api.js`;s.onload=resolve;s.onerror=()=>reject(new Error('Não foi possível carregar o Jitsi as a Service.'));document.head.appendChild(s);});}
    async entrarNaSala(p){try{const acesso=await obterAcessoJaaS(p.id);await this.carregarJitsi(acesso.appId);document.getElementById('tituloSalaJitsi').textContent=p.titulo;document.getElementById('modalSalaJitsi').hidden=false;const c=document.getElementById('jitsiContainer');c.innerHTML='';this.jitsiApi?.dispose();this.jitsiApi=new window.JitsiMeetExternalAPI('8x8.vc',{roomName:acesso.roomName,jwt:acesso.jwt,parentNode:c,width:'100%',height:'100%',lang:'pt',configOverwrite:{prejoinConfig:{enabled:true},startWithAudioMuted:true,disableDeepLinking:true,disableInviteFunctions:true}});}catch(error){alert(error.message);}}
